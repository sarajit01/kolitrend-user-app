...
class SupportTicketWidget extends StatelessWidget {
  ...
  const SupportTicketWidget(
      {super.key, required this.supportTicketModel, required this.index});
  ...
  Widget build(BuildContext context) {
    return Consumer<SupportTicketController>(
      ...
      return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        if (_willShowDate(
            index, supportTicketController.supportTicketList)) ...[
          ...
          Padding(
            ...
            child: Text(
              DateConverter.getRelativeDateStatus(
                  supportTicketController.supportTicketList?[index].createdAt ??
                      '',
                  context),
              ...
              child: Column(
                ...
                children: [
                  Row(children: [
                    SizedBox(
                        width: 15,
                        child: CustomAssetImageWidget(supportTicketModel.type
                                    ?.toLowerCase() ==
                                'website problem'
                    ...
                    Expanded(
                        child: Text(
                            getTranslated(
                                    _toSnakeCase(supportTicketModel.type ?? ''),
                                    context) ??
                  ...
                  Padding(
                    ...
                    child: Row(
                        ...
                        children: [
                          _ContentLabel(
                              labelText: '${getTranslated('sub', context)}'),
                          ...
                          const SizedBox(
                              width: Dimensions.imageSizeExtraSeventy)
                        ]),
                  ),
                  Row(
                      ...
                      children: [
                        _ContentLabel(
                            labelText: '${getTranslated('priority', context)}'),
                        const SizedBox(width: Dimensions.paddingSizeEight),
                        Expanded(
                            child: Text(
                                getTranslated(
                                        ...
                                        context) ??
                                    supportTicketModel.priority!.capitalize(),
                                style: textBold.copyWith(
  ...
  bool _willShowDate(int index, List<SupportTicketModel>? supportTicketList) {
    ...
    final currentSupportTicketDate =
        DateTime.tryParse(currentSupportTicket?.createdAt ?? '');

    final previousSupportTicket = supportTicketList?[index - 1];
    final previousSupportTicketDate =
        DateTime.tryParse(previousSupportTicket?.createdAt ?? '');
...
class _ContentLabel extends StatelessWidget {
  ...
  const _ContentLabel({required this.labelText, this.labelTextStyle});
