...
class _BrandAndCategoryProductScreenState
  ...
  void _initializeCategoryList(String categoryName) {
    ...
    if ((widget.subCategory?.subSubCategories?.isNotEmpty ?? false) &&
        widget.id != null) {
      subSubCategoryList = widget.subCategory?.subSubCategories
              ?.map((element) => element)
              .toList() ??
          [];
      subSubCategoryList?.insert(
          0,
          SubSubCategory(
            name: categoryName,
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) {
        int index = subSubCategoryList!
            .indexWhere((category) => category.id == widget.id);
        if (index != -1) {
        ...
        widget.id != null) {
      subCategoryList = widget.categoryModel?.subCategories
              ?.map((element) => element)
              .toList() ??
          [];
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) {
        int index =
            subCategoryList!.indexWhere((category) => category.id == widget.id);
        if (index != -1) {
          _horizontalScrollToIndex(index);
        }
      });
  ...
  void _getProducts(int? id) async {
    await Provider.of<ProductController>(context, listen: false)
        .initBrandOrCategoryProductList(
            isBrand: widget.isBrand, id: id, offset: 1, isUpdate: false);
  }
  ...
  void initState() {
    if (widget.id != null) {
      Provider.of<ProductController>(context, listen: false)
          .updateSelectedCategoryId(id: widget.id!, isUpdate: false);
    }
                  ...
                  child: ListView.separated(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return _CategoryItemWidget(
                        ...
                        totalProductCount:
                            '(${subCategoryList?[index].totalProductCount.toString() ?? ''})',
                        isSelected: isSubCategorySelected,
                        onTap: () {
                          if (productController.selectedCategoryId !=
                              subCategoryList?[index].id) {
                            debounceHelper.run(() {
                              productController.updateSelectedCategoryId(
              ...
              Padding(
                ...
                child: SizedBox(
                  ...
                  child: ListView.separated(
                    ...
                    itemBuilder: (context, index) {
                      final bool isSubSubCategorySelected =
                          subSubCategoryList?[index].id ==
                              productController.selectedCategoryId;

                      return _CategoryItemWidget(
                        categoryName: subSubCategoryList?[index].name ?? '',
                        totalProductCount:
                            '(${subSubCategoryList?[index].totalProductCount.toString() ?? ''})',
                        ...
                        onTap: () {
                          if (productController.selectedCategoryId !=
                            ...
                            debounceHelper.run(() {
                              productController.updateSelectedCategoryId(
                                  id: subSubCategoryList![index].id!);
...
class _CategoryItemWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return InkWell(
      ...
      child: Container(
        padding: const EdgeInsets.symmetric(
            horizontal: Dimensions.paddingSizeEight, vertical: 6),
            ...
            Text(totalProductCount,
                style: isSelected
                    ? textBold.copyWith(
                        fontSize: Dimensions.fontSizeExtraSmall,
                        color:
                            Theme.of(context).cardColor.withValues(alpha: 0.70))
                    : textRegular.copyWith(
                        fontSize: Dimensions.fontSizeExtraSmall,
                        color: Theme.of(context)
                            .textTheme
