...
class ProductRepository extends DataSyncService
  ...
  @override
  Future<ApiResponseModel> getFilteredProductList(
      BuildContext context, String offset, ProductType productType) async {
    late String endUrl;
  ...
  @override
  Future<ApiResponseModel<T>> getFindWhatYouNeed<T>(
      {required DataSourceEnum source}) async {
    return await fetchData<T>(AppConstants.findWhatYouNeed, source);
  }

  @override
  Future<ApiResponseModel> getJustForYouProductList(
      {required int offset, int? limit}) async {
    try {
      final response = await dioClient
          .get('${AppConstants.justForYou}&limit${limit ?? 10}&offset=$offset');
      return ApiResponseModel.withSuccess(response);
    } catch (e) {
      return ApiResponseModel.withError(ApiErrorHandler.getMessage(e));
    }
  ...
  @override
  Future<ApiResponseModel<T>> getMostSearchingProductList<T>(
  ...
  @override
  Future<ApiResponseModel<T>> getHomeCategoryProductList<T>(
      {required DataSourceEnum source}) async {
    return fetchData(AppConstants.homeCategoryProductUri, source);
  }

  @override
  Future<ApiResponseModel<T>> getClearanceAllProductList<T>(
      {required int offset, required DataSourceEnum source}) async {
    return await fetchData<T>(
        '${AppConstants.clearanceAllProductUri}?guest_id=1&limit=10&offset=$offset',
        source);
  }

  @override
  Future<ApiResponseModel> getClearanceSearchProducts(
      String query,
      String? categoryIds,
      String? brandIds,
      String? authorIds,
