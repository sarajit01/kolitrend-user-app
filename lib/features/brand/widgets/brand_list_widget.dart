  ...
  Widget build(BuildContext context) {
    return Consumer<BrandController>(
      builder: (context, brandProvider, child) {
        ...
        return (brandProvider.brandModel?.brands?.isNotEmpty ?? false) ? widget.isHomePage?
        SingleChildScrollView(scrollDirection: Axis.horizontal, child: Row(
          ...
          child: PaginatedListView(
              ...
              itemView: GridView.builder(
                ...
                itemCount:  brandProvider.brandModel?.brands?.length ?? 0,
                shrinkWrap: true,
                  ...
                  return InkWell(
                    onTap: () {
                      Navigator.push(context, MaterialPageRoute(builder: (_) => BrandAndCategoryProductScreen(
                        isBrand: true,
                        id: brandProvider.brandModel?.brands?[index].id,
                        name: brandProvider.brandModel?.brands?[index].name,
                        image: brandProvider.brandModel?.brands?[index].imageFullUrl?.path,
                      )));
                    },
                    child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
                      Expanded(child: ClipRRect(
                        ...
                        child: Container(
                          ...
                          child: CustomImageWidget(image:'${brandProvider.brandModel?.brands?[index].imageFullUrl?.path!}'),
                        ),
                      )),

                      SizedBox(height: (MediaQuery.of(context).size.width / 4) * 0.3, child: Center(
                        child: Text(
                          brandProvider.brandModel?.brands?[index].name ?? '',
